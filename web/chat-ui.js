/* Ooze Syndicate 2.0 - compact native room chat (Alpha 11 chat-ui.js): reading never opens the keyboard; composing is explicit. Messages carry the host-assigned seat ("who"), rendered as text. The game draws its own CHAT button (lobby, under PAUSE) and calls OozeChat.show(); the DOM toggle stays hidden. */
(()=>{
 ...
 function render(){if(!log||!panel)return;const bottom=log.scrollHeight-log.scrollTop-log.clientHeight<48;log.replaceChildren();for(const m of JSON.parse(last||'[]')){const row=document.createElement('p'),name=document.createElement('strong'),body=document.createElement('span');name.textContent=(m.seat===local?'You':String(m.who))+':';body.textContent=String(m.text);row.append(name,body);log.append(row)}if(bottom)log.scrollTop=log.scrollHeight;unread=0;toggle.textContent='Chat'}
 ...
 toggle.onclick=open;
 window.OozeChat={sync(active,history,seat){if(!toggle.isConnected)document.body.append(toggle);toggle.hidden=true;local=seat;if(!active){close();last='';seen=0;unread=0;pending='';toggle.textContent='Chat';return}if(history!==last){const entries=JSON.parse(history);const latest=entries.length?Number(entries.at(-1).id):0;if(!panel)unread+=entries.filter(m=>Number(m.id)>seen&&m.seat!==local).length;seen=latest;last=history;toggle.textContent=unread?'Chat ('+unread+')':'Chat';render()}},takeMessage(){const value=pending;pending='';return value},show(){if(!panel)open()},unread(){return unread},error(message){if(status&&panel)status.textContent=message}};
})();
