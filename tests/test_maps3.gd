extends SceneTree
## Maps 3.0 check:  Godot --headless --path . --script res://tests/test_maps3.gd
...
##   or plaza unless it is raised, no bridge too short for a deck, piers within the kit's 80 degrees,
##   the builder's own verification is clean, the plaza GLB has every plate and plaza pier;
...
func _layout(m: Dictionary, path: String) -> void:
	...
	for k in range(geo.size()):
		...
		check(float(g["L"]) - float(g["p0"]) - float(g["p1"]) >= 0.5, "%s: deck %d long enough for its pieces" % [code, k])
		check(absf(float(g["lean0"])) <= 82.5 and absf(float(g["lean1"])) <= 82.5, "%s: deck %d piers within the kit's 80 degrees" % [code, k])
...
func _run() -> void:
	...
	check(pool.size() == 109, "maps 3.0: 100 maps + 9 debug maps in the pool (%d)" % pool.size())
	for path in pool:
		var m := MapBuilder.load_map(path)
