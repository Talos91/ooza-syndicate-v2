...
- 0.18.7 went live from 96a87bd (published early); 0.18.8 (f0bcc5b) added skills-fx (`scripts/skill_fx.gd`),
  look-upgrade (`Scenery.build_environment`, `shaders/kit_glass|void_mist|backdrop.gdshader`, kit re-export via
  `Models/2.0/export_kit_2_0.py`) and balance-study (`Rules.BALANCE_PRESETS`, `tests/balance_probe.gd`, `b187` off).
- **Still to do:** the mobile UI / UX pass (branch `ui-pass`, Sonnet agent) (Daniele: "consistent, functional, cool, properly sized,
